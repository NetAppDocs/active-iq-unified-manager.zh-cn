...
|===
. 如果安装的MySQL版本低于MySQL 8.4.7社区版，请输入以下命令进行卸载：
+
...
+
对于 Red Hat Enterprise Linux 8 系统，如果您有包含 JDK 17 - OpenJDK、7-zip 以及 AppStream 存储库提供的其他软件包的内部存储库，则必须禁用 AppStream 存储库并安装 MySQL Community Server。运行以下命令：
