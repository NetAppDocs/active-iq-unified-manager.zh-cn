...
[role="lead"]
您可以通过在 Linux 平台上下载并运行安装文件从 Unified Manager 9.14 或 9.16 升级到 9.18。
...
* 在升级 Unified Manager 之前，必须先安装或升级到正确版本的 7-zip。看 link:task_configure_7zip_package.html["配置 7-zip 软件包"]。
...
* MySQL 社区版会在 Unified Manager 升级期间自动升级。如果您系统上安装的 MySQL 版本早于 8.4.7，则 Unified Manager 升级过程会自动将 MySQL 升级到 8.4.7。
