...
Unified Manager可使用 `yum`实用程序安装软件和任何相关软件。由于Red Hat Enterprise Linux的映像各不相同、因此安装的软件包取决于映像中的软件。该 `yum`实用程序将确定要安装的相关软件包。如果需要有关相关软件包的详细信息，请参见link:../install-linux/reference_red_hat_and_centos_software_and_installation_requirements.html["Linux 软件和安装要求"]。
