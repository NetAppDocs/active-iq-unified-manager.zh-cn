...
您可以更新版本系列中的 OpenJDK 版本。例如、您可以从OpenJDK 11.0.14升级到OpenJDK 11.0.16、但不能直接从OpenJDK 11更新到OpenJDK 12。

.步骤
. 以管理员用户身份登录到 Unified Manager 主机。
. 将适当版本的 OpenJDK （ 64 位）从 OpenJDK 站点下载到目标系统。
+
例如，从下载 `openjdk-11_windows-x64_bin.zip http://[]。
...
. 展开 `zip` 文件。
. 从生成的复制目录和文件 `jdk` 目录(例如、 `jdk-11.0.16` 到Java的安装位置。示例 `C:\Program Files\NetApp\JDK\`
